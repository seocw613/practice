...
public class CommentController {
	...
	public ResponseEntity<String> delete(@RequestBody CommentVO cvo, HttpServletRequest request){
		log.info("comment delete cvo : "+cvo.toString());
		HttpSession ses = request.getSession();
		UserVO uvo = (UserVO)ses.getAttribute("ses");
		if(uvo==null) return new ResponseEntity<String>("1", HttpStatus.INTERNAL_SERVER_ERROR);
		if(!uvo.getId().equals(cvo.getWriter())) {
			return new ResponseEntity<String>("1", HttpStatus.INTERNAL_SERVER_ERROR);
		}
		int isOk = csv.delete(cvo);
		
		return isOk>0? new ResponseEntity<String>("0", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
