...
public class BoardServiceImpl implements BoardService {
	...
	public int register(BoardDTO bdto) {
		...
		if(isOk>0 && bdto.getFList() != null) {  // insert가 되고, 파일이 있는 경우 실행
	...
	public BoardDTO getDetailFile(int bno) {
		...
		BoardDTO bdto = new BoardDTO(bdao.selectDetail(bno), fdao.selectFileList(bno));
		return bdto;
	}
	...
	public int modify(BoardDTO bdto, UserVO user) {
		...
		BoardVO tmpBoard = bdao.selectDetail(bdto.getBvo().getBno());
		if(user == null || !user.getId().equals(tmpBoard.getWriter())) {
			return 0;
		}
		isOk = bdao.updateBoard(bdto.getBvo());
		if(isOk>0 && bdto.getFList() != null) {  // modify가 되고, 파일이 있는 경우 실행
			int bno = bdto.getBvo().getBno();  // 현재 글의 bno 가져오기
			for(FileVO fvo : bdto.getFList()) {
				fvo.setBno(bno);
				log.info("insert file : "+fvo.toString());
				isOk *= fdao.insertFile(fvo);
	...
	public int deleteFile(String uuid) {
		log.info("board fileRemove check 2");
